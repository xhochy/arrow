...
Status FileReader::Impl::GetFlatColumn(int i, std::unique_ptr<FlatColumnReader>* out) {
  if (!CheckForFlatColumn(reader_->metadata()->schema_descriptor()->Column(i))) {
    return Status::Invalid("The requested column is not flat");
  }
  std::unique_ptr<FlatColumnReader::Impl> impl(
      new FlatColumnReader::Impl(pool_, reader_->metadata()->schema_descriptor()->Column(i), reader_.get(), i));
...
Status FileReader::Impl::ReadFlatTable(std::shared_ptr<Table>* table) {
  const std::string& name = reader_->metadata()->schema_descriptor()->schema()->name();
  std::shared_ptr<Schema> schema;
  RETURN_NOT_OK(FromParquetSchema(reader_->metadata()->schema_descriptor(), &schema));

  std::vector<std::shared_ptr<Column>> columns(reader_->metadata()->num_columns());
  for (int i = 0; i < reader_->metadata()->num_columns(); i++) {
